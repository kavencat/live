...
class BLiveClient:
    ...
    for cmd in (  # 其他已知命令
        ...
        'WARNING', 'WEEK_STAR_CLOCK', 'WELCOME', 'WELCOME_GUARD', 'WIN_ACTIVITY', 'INTERACT_WORD','WISH_BOTTLE'
    ):
    ...
    async def _message_loop(self):
        ...
        while True:
            ...
            retry_count += 1
            logger.warning(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' '+'room %d 掉线重连中%d', self.room_id, retry_count)
            try:
            ...
            if header.operation == Operation.HEARTBEAT_REPLY:
                ...
                if header.ver == WS_BODY_PROTOCOL_VERSION_DEFLATE:
                    ...
                    try:
                        ...
                        logger.error(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' '+'body: %s', body)
                        raise
                ...
                body = data[offset + HEADER_STRUCT.size: offset + header.pack_len]
                logger.warning(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' '+'room %d 未知包类型：operation=%d %s%s', self.room_id,
    ...
    async def _handle_command(self, command):
        ...
        if cmd in self._COMMAND_HANDLERS:
            ...
            logger.warning(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+' '+'room %d 未知命令：cmd=%s %s', self.room_id, cmd, command)
